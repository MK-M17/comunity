...
from .forms import CommentForm, UserCreateForm, UserEditForm
...
from  .forms import PostPointForm
...
def post_point_edit(request, post_point_id):
    post_point = get_object_or_404(PostPoint, id=post_point_id)
    post =  get_object_or_404(Post, id=post_point.post.id)
    post_point_edit = PostPointForm(instance=post_point)
    if request.method == "POST":
        post_point_edit = PostPointForm(request.POST, request.FILES, instance=post_point)
        if post_point_edit.is_valid():
            post_point_edit.save()
    return render(request, 'blog/account/post_point_edit.html',
                  {'form': post_point_edit, "post_point":post_point, 'post': post})
...
def like_post(request, post_id):
    ...
    if request.user in post.likes.all():
        post.likes.remove(request.user)  # прибрати лайк
    else:
        post.likes.add(request.user)     # поставити лайк
